...
def longest_streak(str)
    ...
    (0...str.length).each do |i|
        tmp += str[i]
        if tmp[0] != str[i+1]
            longest = tmp if tmp.length >= longest.length
            tmp = ""
        end
    end
    longest
...
def vigenere_cipher(word, offset)
    alpha = ('a'..'z').to_a
    new_message = ""

    word.each_char.with_index do |c, i|
        old = alpha.index(c)
        key = offset[i % offset.length]
        pos = (old + key) % alpha.length
        new_message += alpha[pos]
    end
    new_message
end
...
def vowel_rotate(str)
    ...
    indexes = (0...str.length).select { |i| vowels.include?(str[i]) }
    rotated_vowel_idxs = indexes.rotate(-1)

    indexes.each_with_index do |index, i|
        new_vowel = str[rotated_vowel_idxs[i]]
        new_str[index] = new_vowel
    end
...
class String
    def select(&prc)
        return "" if prc.nil?
        str = ""
        self.each_char {|e| str += e if prc.call(e)}
        str
    end
